...
| 0057 | `0057_cercle_floor_plan.sql` | **Vague V1 — Squad G (Le Cercle)** : seed idempotent de 14 tables `venue_tables` pour `cercle` — modèle de salle PROPRE (8 salon canapés VIP + 4 hautes debout + 2 alcôves modulables, capacité assise 52), distinct de Terminus (18) et Eden (44). Positions en pourcentage (viewBox-agnostique). N'altère aucune ligne Eden/Terminus. `venues('cercle')` déjà présent (0004). `0057_cercle_floor_plan_verification.sql`. | 0057 |

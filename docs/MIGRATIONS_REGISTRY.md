...
| 0074 | `0074_invite_link_revocation.sql` | **Révocation explicite des liens de parrainage** (distincte de l'expiration) : colonnes `revoked_at`/`revoked_by`/`revocation_reason` sur `invite_links` + `revoke_invite_link_v1(link_id, reason)` (promoteur → SES liens ; direction → tous ; autres rôles & anon refusés ; idempotent). `get_invite_link_public` expose `revoked` (UI /i affiche « lien révoqué »). `onboard_referral_v1` refuse un lien révoqué AVANT `register` (aucun guest, aucune résa, aucune consommation `max_uses`, attribution inchangée, même si `expires_at` futur). SECURITY DEFINER search_path figé. Vérifié niveau 4 LABO. `0074_invite_link_revocation_verification.sql`. | 0074 |
